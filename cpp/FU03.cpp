...
//	//FU03_sub_14174();
//
//	WFLAG(FUEL_CUT_FLAG_FFFF8A5E, FCF_KNOCK_ENRICH, (wMUTD1_BitMap_FAA & FAA_7_HIGH_IGN) && wMUT1C_ECU_Load >= word_180C/*80*/ && MUT21_RPM_x125div4 >= word_180A/*128*/);
//
//	FU03_HI_LO_Octan();
//
//	// FU03_sub_14300()
...
void FU03_root_sub()
{
	...
	FU03_sub_159DC();
	FU03_InjLatencyUpdate();
	FU03_sub_15E14();	// REVLIM FUEL_CUT
	FU03_sub_15E48();	// STALL
	FU03_sub_15F28();	// 
	FU03_ASYNHACCELENRICH_sub_1623E();
	FU03_sub_16750();
	FU03_Cranking_Inject();
...
static void FU03_sub_13CE4_hook()
{
	...
	WFLAG(FUEL_CUT_FLAG_FFFF8A5E, FCF_KNOCK_ENRICH, (wMUTD1_BitMap_FAA & FAA_7_HIGH_IGN) && wMUT1C_ECU_Load >= word_180C/*80*/ && MUT21_RPM_x125div4 >= word_180A/*128*/); //FU03_sub_14174();
...
static bool FU03_Check_13E4A()
{
	TRG(wMUT19_Startup_Check_Bits, 0x20, ECU_Load_1, word_16AE/*61*/, word_16AC/*64*/);

	return (wMUT19_Startup_Check_Bits & 0x20) == 0 && (wMUT19_Startup_Check_Bits & 0xF) == 0 && (wMUT10_Coolant_Temperature_Scaled > word_16AA/*103(63)*/);
}
...
static bool FU03_sub_13EE6()
{
	...
	u32 r13 = Sub_Lim_0(r8, leanSpoolEnableLoadHysteresis/*117*/);

	TRG(SPEED_FLAGS, SPD_11_800, ECU_Load_1, r13, r8);

	if (SPEED_FLAGS & SPD_11_800)
	{
		if (MUT21_RPM_x125div4 >= leanSpoolStopRPM/*192(6000)*/)
		{
			r1 = 0xFF;
...
static void FU03_Coolant_Air_Calcs_sub_14088(/*EnVars* ev*/)
{
	...
	if (byte_104E/*1*/ == 0 || (wMUT1E_MAF_RESET_FLAG & STALL) || (wMUT1E_MAF_RESET_FLAG & UPDATE_OXIGEN_TRIM) || ((wMUTD1_BitMap_FAA & FAA_4_CLOSED_LOOP) == 0 && cranking_end_timer_up >= (word_1A1E/*120*/ * 20)))
	{
		CLR(r13, FCF_CRANKING_OVERTEMP);
	}
	else if (wMUT1E_MAF_RESET_FLAG & CRANKING) 
	{
		WFLAG(r13, FCF_CRANKING_OVERTEMP, wMUT10_Coolant_Temperature_Scaled > t1_unk_some_Coolant_Temp_1A16/*143(103)*/ && wMUT11_Intake_Air_Temperature_Scaled > t1_unk_some_Air_Temperature_1A18/*106(66)*/);
	};
...
static void FU03_HI_LO_Octan_stock()
{
	...
	u32 r13 = Query_byte_2D_3D_Table((RT_FLAG1_FFFF8888 & DRIVE_ALWAYS_1) ? LowOctFMp_7AA8 : LowOctFMp_7AA8);

	if (wMUTD1_BitMap_FAA & FAA_7_HIGH_IGN)
	{
		u32 rh = Query_byte_2D_3D_Table((RT_FLAG1_FFFF8888 & DRIVE_ALWAYS_1) ? HIGHOKTF_7A88 : HIGHOKTF_7A88);
...
static void FU03_VE_map_sub_14620_stock()
{
...
static void FU03_VE_map_sub_14620()
{
	...
	wMUT31_Volumetric_Efficiency = ((u32)Table_Lookup_word_2D_3D(veMapArray[veMapIndex&7])+128)>>8;

	//__VE16 = Table_Lookup_word_2D_3D(veMapArray[veMapIndex&7]);

	//wMUT31_Volumetric_Efficiency = ((u32)__VE16 + 128) >> 8;
}
...
static void FU03_Fuel_Knock_Reaction(/*EnVars* ev*/)
{
	...
	else 
	{
		...
		else
		{
			...
			if ((FUEL_CUT_FLAG_FFFF8A5E & FCF_KNOCK_ENRICH)/* && !sub_21E2A()*/)
			{
		...
		if (FUEL_CUT_FLAG_FFFF8A5E & FCF_CRANKING_OVERTEMP)
		{
			u32 r9 = word_1A1E/*120*/ * 20;

			r9 = Mul_Div_R(Sub_Lim_0(r9, cranking_end_timer_up), word_1A1A/*41*/, r9 << 2);
...
static void FU03_sub_Oxygen_Feedback_Trim_stock(/*EnVars* ev*/)
{
...
//	u32 r8 = (!sub_21D9C()) ? FU03_sub_1525A(ev) : 4;
	...
	else
	{
		...
		else if (FUEL_CUT_FLAG_FFFF8A5E & FCF_CRANKING_OVERTEMP)
		{
...
//#pragma noregsave(FU03_sub_Oxygen_Feedback_Trim)
...
static void FU03_sub_Oxygen_Feedback_Trim(/*EnVars* ev*/)
{
	...
	u16 *w1 = &word_FFFF92B6;
	u16 *MUT1E = &wMUT1E_MAF_RESET_FLAG;


	if (*MUT1E & (STALL|CRANKING))
	{
		*w1 = 0x8080;
	}
	else if ((timeEvents & EVT_2_100ms) && (word_FFFF928E & 2))
	{
		*w1 = Interpolate_256(*w1, *r2, (word_FFFF8D58 & 0x2000) ? word_215A : word_1DAA);
	};
...
//	u32 r8 = (!sub_21D9C()) ? FU03_sub_1525A(ev) : 4;
	...
	else
	{
		...
		u32 r13 = 0x8080;
		u32 r3 = word_FFFF929A;
		u16 *FCA = &bMUTD3_BitMap4_FCA_Store_FFFF89D8;
...
static u16 FU03_sub_15C22()
{
	if (ONE(wMUT1E_MAF_RESET_FLAG, STALL) || ZRO(RT_FLAG1_FFFF8888, STARTER))
	{
...
static void FU03_sub_161FC()
{
	...
	word_FFFF8B20 = Table_Lookup_byte_2D_3D((RT_FLAG1_FFFF8888 & DRIVE_ALWAYS_1) ? unk0523_361E : unk0524_362C);
}
